                    ...
                    double scale)
{
	...
	if(!eyesWithoutAFace(face,eye,radius))
	{
	    ...
	    system("clear");
	    cout << "DISPLAY: Senha/codigo: ";
	    cin >> password;
	    cout << "DISPLAY: Aguarde... ";
	    sprintf(comando, "bash executar_models.sh %s", password.c_str());
	    system(comando); sleep(10);
	    sprintf(comando, "bash mageyBot_what.sh 5994091183:AAG4Zr57KCKglxo2fV5qqGbpcXYEOnM1Ma8 %s", password.c_str());
	    system(comando);
	} else 
